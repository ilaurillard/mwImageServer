...
class ChildConsignment {
  ...
  static ChildConsignment? fromXml(XmlElement? xml) {
  ...
  XmlNode toXml() {
    ...
    List<XmlNode?> c2 = [
      uBLExtensions?.toXml(),
      iD.toXml(),
      carrierAssignedID?.toXml(),
      consigneeAssignedID?.toXml(),
      consignorAssignedID?.toXml(),
      freightForwarderAssignedID?.toXml(),
      brokerAssignedID?.toXml(),
      contractedCarrierAssignedID?.toXml(),
      performingCarrierAssignedID?.toXml(),
      ...summaryDescription.map((SummaryDescription e) => e.toXml()).toList(),
      totalInvoiceAmount?.toXml(),
      declaredCustomsValueAmount?.toXml(),
      ...tariffDescription.map((TariffDescription e) => e.toXml()).toList(),
      tariffCode?.toXml(),
      insurancePremiumAmount?.toXml(),
      ...
      loadingLengthMeasure?.toXml(),
      ...remarks.map((Remarks e) => e.toXml()).toList(),
      hazardousRiskIndicator?.toXml(),
      animalFoodIndicator?.toXml(),
      humanFoodIndicator?.toXml(),
      livestockIndicator?.toXml(),
      bulkCargoIndicator?.toXml(),
      containerizedIndicator?.toXml(),
      generalCargoIndicator?.toXml(),
      specialSecurityIndicator?.toXml(),
      thirdPartyPayerIndicator?.toXml(),
      ...carrierServiceInstructions.map((CarrierServiceInstructions e) => e.toXml()).toList(),
      ...customsClearanceServiceInstructions.map((CustomsClearanceServiceInstructions e) => e.toXml()).toList(),
      ...forwarderServiceInstructions.map((ForwarderServiceInstructions e) => e.toXml()).toList(),
      ...specialServiceInstructions.map((SpecialServiceInstructions e) => e.toXml()).toList(),
      sequenceID?.toXml(),
      ...
      consolidatableIndicator?.toXml(),
      ...haulageInstructions.map((HaulageInstructions e) => e.toXml()).toList(),
      loadingSequenceID?.toXml(),
      childConsignmentQuantity?.toXml(),
      totalPackagesQuantity?.toXml(),
      ...consolidatedShipment.map((ConsolidatedShipment e) => e.toXml()).toList(),
      ...customsDeclaration.map((CustomsDeclaration e) => e.toXml()).toList(),
      requestedPickupTransportEvent?.toXml(),
      requestedDeliveryTransportEvent?.toXml(),
      plannedPickupTransportEvent?.toXml(),
      plannedDeliveryTransportEvent?.toXml(),
      actualPickupTransportEvent?.toXml(),
      actualDeliveryTransportEvent?.toXml(),
      ...status.map((Status e) => e.toXml()).toList(),
      ...childConsignment.map((ChildConsignment e) => e.toXml()).toList(),
      ...
      notifyParty?.toXml(),
      originalDespatchParty?.toXml(),
      finalDeliveryParty?.toXml(),
      performingCarrierParty?.toXml(),
      substituteCarrierParty?.toXml(),
      logisticsOperatorParty?.toXml(),
      transportAdvisorParty?.toXml(),
      hazardousItemNotificationParty?.toXml(),
      insuranceParty?.toXml(),
      mortgageHolderParty?.toXml(),
      billOfLadingHolderParty?.toXml(),
      originalDepartureCountry?.toXml(),
      finalDestinationCountry?.toXml(),
      ...transitCountry.map((TransitCountry e) => e.toXml()).toList(),
      transportContract?.toXml(),
      ...transportEvent.map((TransportEvent e) => e.toXml()).toList(),
      originalDespatchTransportationService?.toXml(),
      finalDeliveryTransportationService?.toXml(),
      deliveryTerms?.toXml(),
      paymentTerms?.toXml(),
      collectPaymentTerms?.toXml(),
      disbursementPaymentTerms?.toXml(),
      prepaidPaymentTerms?.toXml(),
      ...freightAllowanceCharge.map((FreightAllowanceCharge e) => e.toXml()).toList(),
      ...extraAllowanceCharge.map((ExtraAllowanceCharge e) => e.toXml()).toList(),
      ...mainCarriageShipmentStage.map((MainCarriageShipmentStage e) => e.toXml()).toList(),
      ...preCarriageShipmentStage.map((PreCarriageShipmentStage e) => e.toXml()).toList(),
      ...onCarriageShipmentStage.map((OnCarriageShipmentStage e) => e.toXml()).toList(),
      ...transportHandlingUnit.map((TransportHandlingUnit e) => e.toXml()).toList(),
      firstArrivalPortLocation?.toXml(),
      lastExitPortLocation?.toXml(),
      officeOfEntryLocation?.toXml(),
      officeOfSubSequentiallyEntryLocation?.toXml(),
      officeOfExitLocation?.toXml(),
      officeOfDepartureLocation?.toXml(),
      officeOfDestinationLocation?.toXml(),
