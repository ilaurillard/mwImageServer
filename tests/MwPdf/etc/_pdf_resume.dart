...
Future<void> main() async {
  Uint8List pdf =
      await generateResume(PdfPageFormat.a4, CustomData(name: 'Peter'));

  final file = File('_resume.pdf');
...
Future<Uint8List> generateResume(PdfPageFormat format, CustomData data) async {
  ...
  final profileImage = pw.MemoryImage(
      await File('tests/MwPdf/etc/files/profile.jpg').readAsBytes());
...
Future<pw.PageTheme> _myPageTheme(
  ...
  pw.Font openSansRegular = pw.Font.ttf(
    ByteData.view(
      (await File('lib/MwPdf/res/fonts/OpenSans/OpenSans-Regular.ttf').readAsBytes())
          .buffer,
    ),
  );
